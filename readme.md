...
**Study context:** This repository accompanies an analysis exploring how coping strategies mediate the relationship between socio‑demographic factors and mental health outcomes (depression and anxiety) in Iraqi parents caring for children with autism spectrum disorder (ASD). All participant‑level data are de‑identified and not shared publicly.
...
This repository contains `Bayesian_SEM.Rmd` — an R Markdown script that performs data preprocessing, exploratory analyses of background variables, confirmatory factor analysis of Brief‑COPE items, and a Bayesian structural equation modelling (BSEM) pipeline. The aim is to examine whether coping strategies (problem‑focused, emotion‑focused and avoidant) mediate the association between background variables (e.g., age, socioeconomic status, family characteristics) and parental mental health outcomes (PHQ‑9 depression and GAD‑7 anxiety).

> **Data privacy:** The raw survey responses are **not included** for confidentiality. Users must supply their own dataset in a CSV file named `input.csv` with the required columns (see below).
...
- **`data collection tool/`** — Folder containing the Arabic and English questionnaires and a scored version (DOCX).  
...
|----------------------------------|---------------|
| Continuous background variables   | `age`, `ses`, `no_child`, `no_asd_child`, `c_age`, `time_since`, `delay`【640811584478314†L34-L52】 |
| Ordinal background variable       | `birth_rank`【640811584478314†L54-L55】 |
| Nominal background variables      | `father`, `marital_stat`, `residence`, `consanguinity`, `living_sit`, `parent_cond`, `c_gender`, `comorbidity`【640811584478314†L69-L77】 |
| Coping items (Brief‑COPE)         | `sd`, `ac`, `de`, `es`, `is`, `bd`, `ve`, `pr`, `pl`, `hu`, `acc`, `re`, `sb`【640811584478314†L34-L38】 |
| Outcomes                          | `phq9`, `gad7`【640811584478314†L34-L38】 |
...
- **Data preprocessing:** Standardises continuous variables and computes Spearman correlations for numeric/ordinal background variables【640811584478314†L34-L63】.  
- **Exploratory tests:** Examines relationships between nominal and numeric/ordinal variables using Kruskal–Wallis and chi‑square tests, and computes effect sizes (η² and Cramer’s V)【640811584478314†L69-L199】.  
- **Confirmatory factor analysis (CFA):** Specifies a three‑factor model for Brief‑COPE (problem‑focused, emotion‑focused, avoidant)【640811584478314†L221-L230】 and fits the model using maximum likelihood.  
