---
title: "Mediation of Coping on Parents' Depression and Anxiety: Reproducible Report"
output:
  pdf_document:
    toc: true
...
lavaan, blavaan, rstan : Perform Bayesian Structural Equation Modelling.

dplyr and tibble       : Perform data manipulation and tabulation.

ggplot2                : Perform data visualization

```{r}
packages <- c("lavaan","blavaan","rstan","dplyr", "tibble", "ggplot2")
...
cont_vars <- c("age","ses","c_age","time_since","delay",
               "sd","ac","de","es","is","bd","ve","pr",
               "pl","hu","acc","re","sb", "phq9","gad7")
data[cont_vars] <- scale(data[cont_vars])
```

## Check the correlation between numeric and ordinal background variables
...
as.data.frame(round(cor_spearman, 3))
```

\newpage

## Check the association between nominal background variables
...
results <- do.call(rbind, lapply(nom_vars, function(nom) {
  ...
  for (v in ord_vars) {
    ...
    effect_label <- as.character(cut(V,
                                     breaks = c(-Inf, 0.10, 0.20, 
                                                0.40, 0.60, 0.80, Inf),
                                     labels = c("Negligible", "Weak", 
                                                "Moderate", "Relatively strong", 
                                                "Strong", "Very strong"),
                                     right  = FALSE
                                     )
                                 )
...
# Re-order columns
results <- results[, c("nominal_var","other_var","test","statistic","p_value",
                       "p_adj_bh","effect_size","effect_label")]
...
# Perform bayesian structural equation modelling
...
diagnostics <- function(fit_bayes) {
  ...
  diagnostics_df <- tibble(Diagnostic = c("Divergences", "Treedepth Hits", 
                                          "Min BFMI", "Max R-hat", "Min ESS"),
                           Value      = c(total_div, td_rate, min_bfmi, 
                                          max_rhat, min_ess),
...
filter_paths <- function(paths_df,
                         pd_threshold = 0.95,
                         coping_vars = c("sd","ac","de","es","is","bd","ve",
                                         "pr","pl","hu","acc","re","sb"),
                         background_vars = c("age","father","ses","residence",
                                             "consanguinity","living_sit", 
                                             "no_child","parent_cond","time_since",  
                                              "delay","c_gender","comorbidity"),                              
                         outcome_vars = c("phq9","gad7")) {

  # Keep paths with probability of direction >=0.95 and no 0 in 95% credible interval
  base_df <- paths_df %>%
  ...
  if (length(retained_copings) == 0) {
    stop("No coping strategies meet the Coping -> Outcome criteria.")
  }
...
### Acquire draws from the posterior distributions
...
apply_pipeline <- function(model, data, beta_vec){
  ...
  # Fit models, calculate diagnostics, extract paths, and get posteriors draws
  ...
  if (nrow(large_paths) > 0) {
    ...
    sensitivity$large_plot <- ggplot(large_summary,
                                     aes(x = beta_sd, y = mean, group = interaction(from, to))) +
      geom_line() +
      geom_point() +
      geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd), width = 0.0) +
      facet_wrap(~ paste(from, "to", to), scales = "free_y") +
